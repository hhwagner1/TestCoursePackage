...
- **ralu.loci**: Data frame with populations and genetic data (181 rows x 9 columns). Included in package 'TestCoursePackage'. To load it, type: data(ralu.loci)

### c) Required R libraries

All required packages should have been installed already when you installed 'TestCoursePackage'.
...
First, check that your active working directory is the folder you created for this lesson. If not, use the RStudio menu to set it manually: 'Session > Set Working Directory > Choose Directory'. This is where R will look for files and save anything you export.

```{r}
getwd()
```

The dataset 'ralu.loci' is available in the package 'TestCoursePackage'. Here we first export it as a comma separated file (.csv), as this is the recommended format for importing your own data (e.g. from Excel). 
...
Check that the file "ralu.loci.csv" is in your lesson folder. Note that in Excel, the columns with the loci must be explicitly defined as 'text', otherwise Excel is likely to interpret them as times. Instructions: 
...
As the file is saved in csv format, we use the function 'read.csv' to import it into an R object called 'Frogs'. We can inspect it with the function 'View'.
...
### d) Check the dimensions and data type:

Use function 'dim' to check the dimensions (number of rows and columns) of the imported data. The data set has 181 rows (frogs) and 10 columns (variables). 
```{r}
dim(Frogs)
```
