...
As the file is saved in comma-separated file format (.csv), we use the function 'read.csv' to import it into an R object called 'RALU.data'. Here we use 'system.file' to access the raw data file that comes with the course R package.

```{r}
tmp <- read.csv(system.file("extdata", "RALU_loci_12pops.csv", 
...
```{r}
RALU.gstudio <- read_population(system.file("extdata", "RALU_loci_12pops.csv", 
                package = "TestCoursePackage"), type="separated", 
                locus.columns=c(2:9), phased=NULL, sep=",", header=TRUE)
```
